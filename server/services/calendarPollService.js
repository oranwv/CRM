    ...
    for (const ev of events) {
      ...
      await pool.query(`
        INSERT INTO google_calendar_cache
          (google_event_id, title, description, start_time, end_time, all_day, color_id, html_link, fetched_at)
        VALUES ($1,$2,$3,$4,$5,$6,$7,$8,NOW())
        ON CONFLICT (google_event_id) DO UPDATE SET
          title=$2, description=$3, start_time=$4, end_time=$5,
          all_day=$6, color_id=$7, html_link=$8, fetched_at=NOW()
      `, [ev.id, ev.summary || '', ev.description || '', startRaw, endRaw, allDay, ev.colorId || null, ev.htmlLink || null]);
    }

    // Remove events in the fetched window that were deleted from Google Calendar
    if (events.length > 0) {
