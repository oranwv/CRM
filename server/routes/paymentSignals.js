// /api/payment-signals — "תשלום ללא מסמך" (see services/paymentSignals.js)
  ...
  try {
    const { rows } = await pool.query(`
      SELECT s.id, s.lead_id, l.name AS lead_name, l.event_type, l.event_date, l.stage,
             s.amount, s.method, s.snippet, s.said_at, s.detected_at
...
router.get('/lead/:leadId', async (req, res) => {
  try {
    const { rows } = await pool.query(`
      SELECT id, lead_id, amount, method, snippet, said_at FROM payment_signals
