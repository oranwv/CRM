...
for (const k of ['SERVER_URL', 'GOOGLE_WEB_CLIENT_ID', 'GOOGLE_WEB_CLIENT_SECRET']) {
  if (typeof process.env[k] === 'string') process.env[k] = process.env[k].trim();
}
if (process.env.SERVER_URL) process.env.SERVER_URL = process.env.SERVER_URL.replace(/\/+$/, '');
