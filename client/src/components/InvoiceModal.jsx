...
// `initial` (optional): { docType, amount, description, paymentMethod, paymentDate } — e.g. a receipt
...
export default function InvoiceModal({ lead, allPhones, allPhoneLabels, onClose, onCreated, initial = null }) {
  ...
  const [items,         setItems]         = useState([{ description: initial?.description || 'אירוע', quantity: 1, price: initial?.amount ?? '', vatType: 1 }]);
