...
// DriveFilePicker: browse configured Google Drive folders and pick files
// Props:
//   onSelect(files) — called with array of { type:'drive', fileId, name, mimeType }
//   onClose()
export default function DriveFilePicker({ onSelect, onClose }) {
  const [folders, setFolders] = useState([]);
  const [activeFolder, setActiveFolder] = useState(null);
  const [files, setFiles] = useState([]);
  const [selected, setSelected] = useState({}); // fileId → { fileId, name, mimeType }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm flex flex-col" style={{ maxHeight: '80vh' }} onClick={e => e.stopPropagation()} dir="rtl">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
          ...
          {activeFolder && <span className="text-sm font-black text-slate-800 truncate mx-2">{activeFolder.name}</span>}
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 min-h-0">
          ...
          {!loading && !error && activeFolder && (
            <>
              ...
              {files.map(f => (
                <button key={f.id} onClick={() => toggleFile(f)}
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-right transition mb-1 ${selected[f.id] ? 'bg-violet-100 border border-violet-300' : 'hover:bg-slate-50 border border-transparent'}`}>
                  <span className={`w-5 h-5 rounded border flex-shrink-0 flex items-center justify-center text-xs ${selected[f.id] ? 'bg-violet-600 border-violet-600 text-white' : 'border-slate-300'}`}>
                    {selected[f.id] ? '✓' : ''}
                  </span>
