...
// "תשלומים ללא מסמך" — customers (or staff notes) that reported a payment, and no
// receipt / invoice followed. Opened from the amber badge in the header (managers +
// finance). A row opens the lead card, where the banner offers "הפק קבלה".
    ...
    <div className="fixed inset-0 z-[78] flex items-center justify-center bg-black/50 p-4" dir="rtl" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 shrink-0">
          <div>
            <h2 className="font-black text-slate-800 text-base">💸 תשלומים שדווחו ולא הופק מסמך</h2>
            <p className="text-[11px] text-slate-400 mt-0.5">זוהה אוטומטית מהודעות הלקוחות ומהערות הצוות</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          ...
          {!loading && !error && items.length === 0 && (
            <p className="text-center text-slate-400 text-sm py-8">אין תשלומים פתוחים ללא מסמך ✅</p>
          )}
          {items.map(it => (
            <div key={it.id} role="button" tabIndex={0} onClick={() => openLead(it.lead_id)}
              ...
              <div className="flex items-center justify-between gap-2">
                <span className="font-bold text-slate-800 truncate">{it.lead_name}</span>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                {it.event_type || 'אירוע'}{it.event_date ? ` · ${fmt(it.event_date)}` : ''}{it.method ? ` · ${it.method}` : ''} · דווח {fmt(it.said_at)}
              </p>
              {it.snippet && <p className="text-xs text-slate-600 mt-1 wrap-anywhere">„{it.snippet}”</p>}
